...
while 1:
    if datetime.now().second == 0:
        ...
        macd,signal,hist = MACD(df['close'].astype('double'), fastperiod = 12, slowperiod = 26, signalperiod = 9)
        price = float(client.get_all_tickers(symbol=coin)['price'])
        if (hist[99]-hist[98] > 0):
             balance-=10000
             hold_coin+= 10000/price
        elif (hist[99]-hist[98] < 0):
            hold_coin-=10000/price
            balance+=10000
        print('tài sản: ' + str(balance+hold_coin*price) + ', lợi nhuận: ' + str((balance+hold_coin*price)/10000-100) + '%')
        sleep(30)
